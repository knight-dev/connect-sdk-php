...
final class ClientTest extends TestCase
{
    ...
    public function testRecorderReceivesMaskedEntry(): void
    {
        $httpClient = $this->makeHttpClient(
        ...
        $captured = [];
        $client = new Client([
            'apiKey' => 'sk_secret_do_not_leak',
            'baseUrl' => 'https://dev-api.logicware.app',
            'httpClient' => $httpClient,
            'recorder' => function (array $entry) use (&$captured): void {
                $captured[] = $entry;
            },
        ]);

        $client->warehouses->get('w_1');

        $this->assertCount(1, $captured);
        $entry = $captured[0];
        $this->assertSame('GET', $entry['method']);
        $this->assertSame(200, $entry['status']);
        $this->assertArrayHasKey('requestHeaders', $entry);
        $this->assertSame('***', $entry['requestHeaders']['X-Api-Key']);
        $this->assertStringNotContainsString('sk_secret_do_not_leak', json_encode($entry) ?: '');
        $this->assertArrayHasKey('descriptor', $entry);
        $this->assertSame('GET', $entry['descriptor']['method']);
        $this->assertStringStartsWith('/api/v1/warehouses/', (string) $entry['descriptor']['path']);
        $this->assertIsFloat($entry['durationMs']);
        $this->assertNull($entry['error']);
    }

    public function testRawRequestReturnsStatusAndBody(): void
    {
        $httpClient = $this->makeHttpClient(
            fn(): ResponseInterface => new Response(
                418,
        ...
        $result = $client->http->rawRequest(['method' => 'GET', 'path' => '/api/v1/teapot']);

        $this->assertSame(418, $result['status']);
        $this->assertSame('{"code":"teapot"}', $result['body']);
        $this->assertArrayHasKey('request', $result);
        $this->assertSame('***', $result['request']['headers']['X-Api-Key']);
