...
function signupShipper(array $input): array
{
    ...
    try {
        ...
        // Address-provisioning failures — see shipper-signup-flow for the full table
        $code = $e->getErrorCode();
        if ($code === 'ADDRESS_CODE_REQUIRED') {
            return ['ok' => false, 'reason' => 'Please enter a warehouse address code.'];
        }
        if ($code === 'ADDRESS_PREFIX_UNKNOWN' || $code === 'ADDRESS_CODE_FORMAT_INVALID') {
            return ['ok' => false, 'reason' => "That address code doesn't look right. Check the format (e.g. CNW-12345)."];
        }
        if ($code === 'ADDRESS_CODE_CONFLICT') {
            return ['ok' => false, 'reason' => 'That address code is already assigned to another shipper.'];
        }
        if ($code === 'ADDRESS_CODE_IMMUTABLE' || $code === 'ADDRESS_CODE_PREFIX_MISMATCH') {
            // Needs explicit operator intent — do NOT auto-retry with forceAddressCode
            return ['ok' => false, 'reason' => 'This shipper already has a different address. Update requires operator confirmation.'];
        }

        if ($code === 'SHIPPER_CODE_CONFLICT') {
